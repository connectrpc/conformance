...
const SERVER_HOST = `http://${argv.host}:${argv.port}`;
...
function doCustomMetadata(done) {
  ...
  call.on('metadata', (metadata) => {
    var m = caseless(metadata); // http header is case-insensitive
    assert(m.has(ECHO_INITIAL_KEY));
    assert.equal(m.get(ECHO_INITIAL_KEY), ECHO_INITIAL_VALUE);
    done();
  });

  call.on('status', (status) => {
    assert('metadata' in status);
    var m = caseless(status.metadata); // http header is case-insensitive
    assert(m.has(ECHO_TRAILING_KEY));
    assert.equal(m.get(ECHO_TRAILING_KEY), ECHO_TRAILING_VALUE);
    done();
  });
}
...
function doSpecialStatusMessage(done) {
  var testService = new TestServiceClient(SERVER_HOST, null, null);
  var req = new SimpleRequest();
...
function doUnimplementedService(done) {
  var unimplementedService = new UnimplementedServiceClient(SERVER_HOST, null, null);
  unimplementedService.unimplementedCall(new Empty(), {}, (err, response) => {
    assert(err);
    assert('code' in err);
    assert.equal(err.code, 12);
    done();
