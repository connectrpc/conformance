...
func loadConfig(configFileName string) ([]configCase, error) {
...
func resolveFeatures(features *conformancev1alpha1.Features) (supportedFeatures, error) { //nolint:gocyclo
	...
	if len(result.Compressions) == 0 {
		result.Compressions = []conformancev1alpha1.Compression{conformancev1alpha1.Compression_COMPRESSION_GZIP}
	}
	...
	includesHalfDuplex := contains(result.StreamTypes, conformancev1alpha1.StreamType_STREAM_TYPE_FULL_DUPLEX_BIDI_STREAM)
...
func resolveCase(features supportedFeatures, unresolvedCase *conformancev1alpha1.ConfigCase) (map[configCase]struct{}, error) {
	...
	if unresolvedCase.Version != conformancev1alpha1.HTTPVersion_HTTP_VERSION_UNSPECIFIED {
		...
		case conformancev1alpha1.HTTPVersion_HTTP_VERSION_2:
			if !unresolvedCase.UseTls && !features.SupportsH2C {
	...
	return computeCasesFromFeatures(impliedFeatures, []bool{unresolvedCase.UseTls}), nil
}
